...
class AsyncStepProtector {
    ...
    setTimeout( timeout_ms ) {
        ...
        this[LIMIT_EVENT] = async_tool.callLater(
            ( ) => {
                ...
                try {
                    this[ROOT].error( Errors.Timeout );
                } catch ( e ) {
                    // ignore
                }
            },
    ...
    break( label ) {
        ...
        this[ROOT].error( Errors.LoopBreak );
    }
    ...
    continue( label ) {
        this[SANITY_CHECK]();
        this[STATE][LOOP_TERM_LABEL] = label;
        this[ROOT].error( Errors.LoopCont );
    }
    ...
    [SANITY_CHECK]() {
        try {
            const stack = this[ROOT][STACK];

            if ( stack[stack.length - 1] !== this ) {
                this[ROOT].error( Errors.InternalError, "Invalid call (sanity check)" );
            }
        } catch ( _ ) {
